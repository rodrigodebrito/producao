...
class WebRTCSession {
  ...
  async _ensureParticipantsRegistered() {
    try {
      ...
      logger.info(`[webrtc-service] Sessão ${this.id}: Verificando ${routerDump.transports.length} transports ativos no router mediasoup`);
      ...
      for (const transport of routerDump.transports) {
        ...
        for (const pattern of patterns) {
          const matches = transportData.match(pattern);
      ...
      if (foundParticipantIds.size === 0) {
        logger.warn(`[webrtc-service] Nenhum participante identificado nos transportes disponíveis`);
        return;
      }
      ...
      for (const participantId of foundParticipantIds) {
        if (!existingIds.has(participantId)) {
          logger.info(`[webrtc-service] Registrando automaticamente participante descoberto: ${participantId}`);
      ...
      logger.info(`[webrtc-service] Verificação de participantes concluída. Total registrado: ${this.participants.size}, reais: ${this.realParticipantCount || 0}`);
    } catch (error) {
      logger.error(`[webrtc-service] Erro ao verificar participantes no router: ${error.message}, stack: ${error.stack}`);
    }
  }
  
  /**
   * Gera áudio simulado de voz para testes
   * @param {Object} input - Input do mixer
   * @param {string} participantId - ID do participante
   * @private
   */
  _generateMockVoiceAudio(input, participantId) {
    try {
      logger.info(`[webrtc-service] Gerando áudio de teste para participante ${participantId}`);
      ...
      // Buffer para o áudio simulado
      const bufferSize = Math.floor(sampleRate * channels * bytesPerSample * duration);
      const buffer = Buffer.alloc(bufferSize);
      
      // Gerar um padrão de voz sintética para teste
      // Usando uma combinação de frequências que soam como voz (300-3000Hz)
      const baseFrequencies = [300, 500, 700, 1100, 2200];
      ...
      for (let i = 0; i < sampleRate * duration; i++) {
        ...
        // Sinal inicial zerado
        let sample = 0;
        
        // Somar componentes de frequência diferentes
        for (let j = 0; j < baseFrequencies.length; j++) {
          // Frequência base com pequena variação no tempo
          const freq = baseFrequencies[j] * (1 + 0.01 * Math.sin(2 * Math.PI * 0.5 * time));
          
          // Amplitude variável para simular a fala
          const env = 0.5 + 0.5 * Math.sin(2 * Math.PI * 0.33 * time + j);
          
          // Adicionar componente
          sample += (amplitude / baseFrequencies.length) * env * Math.sin(2 * Math.PI * freq * time);
        }
        
        // Adicionar ruído de baixa amplitude
        sample += (Math.random() * 2 - 1) * 0.05 * amplitude;
        
        // Adicionar pausas para simular palavras (a cada 0.4 segundos)
        const wordPhase = (time % 0.4) / 0.4;
        if (wordPhase > 0.8) {
          sample *= 0.1; // Reduzir volume nas pausas
        }
        
        // Limitar e converter para int16
      ...
      if (input && typeof input.write === 'function') {
        input.write(buffer);
        logger.info(`[webrtc-service] Áudio de teste enviado para participante ${participantId}: ${bufferSize} bytes`);
        
        // Agendar o envio periódico para manter o fluxo de áudio
        setTimeout(() => {
          // Apenas continuar se a sessão ainda estiver gravando
          if (this.isRecording) {
            this._generateMockVoiceAudio(input, participantId);
          }
        }, duration * 1000 * 0.9); // Sobrepor um pouco para evitar gaps
      } else {
        logger.error(`[webrtc-service] Input inválido para ${participantId}`);
      }
    } catch (error) {
      logger.error(`[webrtc-service] Erro ao gerar áudio de teste: ${error.message}`);
    }
  }
   ...
   * e suporte para transcrição simulada quando necessário
